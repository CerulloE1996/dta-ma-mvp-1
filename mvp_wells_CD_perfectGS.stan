...
parameters {
              ...
              cholesky_factor_corr[nt] L_Omega_global_d_pre;
              cholesky_factor_corr[nt] L_Omega_global_nd_pre;
...
transformed parameters { 
             ...
             mu[,3] = mu_pre[,2];
      {
        for (s in 1:n_studies) {
                      // between-study model for binary
                 nu[s,  ,1] =  to_vector(mu[,1]); // perfect GS
                 nu[s,  ,2] =  to_vector(mu[,2]) + diag_pre_multiply(sd1[2,], L_Omega_bs[2,,]) * z[s, ,2] ; // d-dimer
                 nu[s,  ,3] =  to_vector(mu[,3]) + diag_pre_multiply(sd1[3,], L_Omega_bs[3,,]) * z[s, ,3] ; // Wells

                R_diff_d_pre[s,]  = multiply_lower_tri_self_transpose(R_diff_L_d_pre[s,]);
                R_diff_nd_pre[s,] = multiply_lower_tri_self_transpose(R_diff_L_nd_pre[s,]);

                R_diff_d[s,1,1]  = 1;  R_diff_d[s,2,2]  = 1;  R_diff_d[s,3,3]  = 1; 
                R_diff_d[s,1,2]  = 0;  R_diff_d[s,2,1]  = 0;   R_diff_d[s,1,3]  = 0;  R_diff_d[s,3,1]  = 0;  
                R_diff_d[s,2,3]  = R_diff_d_pre[s,2,3];  R_diff_d[s,3,2]  = R_diff_d_pre[s,2,3];
                R_diff_nd[s,1,1]  = 1;  R_diff_nd[s,2,2]  = 1;  R_diff_nd[s,3,3]  = 1; 
                R_diff_nd[s,1,2]  = 0;  R_diff_nd[s,2,1]  = 0;   R_diff_nd[s,1,3]  = 0;  R_diff_nd[s,3,1]  = 0;  
                R_diff_nd[s,2,3]  = R_diff_nd_pre[s,2,3];  R_diff_nd[s,3,2]  = R_diff_nd_pre[s,2,3];

                R_diff_L_d[s,] = cholesky_decompose(R_diff_d[s,]);
                R_diff_L_nd[s,] = cholesky_decompose(R_diff_nd[s,]); 

                Omega_global_d_pre  = multiply_lower_tri_self_transpose(L_Omega_global_d_pre);
                Omega_global_nd_pre = multiply_lower_tri_self_transpose(L_Omega_global_nd_pre);

                Omega_global_d[1,1] = 1;  Omega_global_d[2,2] = 1;  Omega_global_d[3,3] = 1; 
                Omega_global_d[1,2] = 0;  Omega_global_d[2,1] = 0;  Omega_global_d[1,3] = 0; Omega_global_d[3,1] = 0; 
                Omega_global_d[3,2] = Omega_global_d_pre[3,2]; Omega_global_d[2,3] = Omega_global_d_pre[2,3]; 
                Omega_global_nd[1,1] = 1;  Omega_global_nd[2,2] = 1;  Omega_global_nd[3,3] = 1; 
                Omega_global_nd[1,2] = 0;  Omega_global_nd[2,1] = 0;  Omega_global_nd[1,3] = 0; Omega_global_nd[3,1] = 0; 
                Omega_global_nd[3,2] = Omega_global_nd_pre[3,2]; Omega_global_nd[2,3] = Omega_global_nd_pre[2,3]; 
                L_Omega_global_d = cholesky_decompose(Omega_global_d); 
                L_Omega_global_nd = cholesky_decompose(Omega_global_nd); 
          ...
          for (n in 1:ns[s]) {
            ...
            for (i in 1:nt) { // loop over each test - make sure binary tests are at the start 
             ...
             // Binary Likelihoods (tests w/ 1 threshold)
              if (i <= num_binary_tests) { //  Binary tests / 1 threshold - Bernoulli likelihoods
             ...
             // Ordinal Likelihoods (tests w/ > 1 threshold)
              else { // Ordinal tests / K thresholds s.t. K > 1 - Categorical likelihoods
...
generated quantities {
...

 {
  ...
  }
    {
        ...
        vector[nt] means_d2[n_studies];
        vector[nt] means_nd2[n_studies];
